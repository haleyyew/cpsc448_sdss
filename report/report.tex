...
Web hits and SQL queries submitted via the access interfaces to the CAS, are logged and  provided in the SkyServer or SDSS log data~\cite{singh2007skyserver,raddick2014ten1,raddick2014ten2}. These logs include information about the IP address submitted the query or web hit, webpage views, web agent, date stamps, the submitted SQL queries, and the data release that was queried~\cite{szalay2002sdss}. 
...
Client IP addresses and agent strings: First agent strings in the logs are associated with Web agents. These web agents are then classified into 5 classes: anonymous, bot, administrative service, user program, or Web browser. This is stored in agentinformation table. 
...
\subsection{Tracing a Session} \textcolor{blue}{Here we need an example for a session, that is, a sequence of hits/queries submitted by a user.  Trace the session, and put relevant hits/queries here in a figure.
Questions to address : \begin{itemize}
...
\end{itemize} }
The challenge\footnote{Why is this a challenge? We should be able to trace the session using the schema. What you need perhaps is an example sql query that joins the tables, and returns the queries in that session.} for studying the SQL usage pattern by sessions is to trace each session to construct the original query submitted. This can be achieved by joining multiple tables in the normalized logs dataset. For example, a session table defines individual sessions and the sessionLogs table stores all requests of all sessions with the rankInSession attribute that orders all requests of each session sequentially. sessionLogs table also stores the necessary pointers to the WebLog and SqlLog tables to retrieve the original query. Studying the results returned by each query assists our usage pattern analysis. However, since the SqlLog does not store the results returned by submitting the query - it only stores the number of rows returned - we will need submit the query to Skyserver to retrieve the contents of the rows.
