...
When joining tables, single each joining condition is a equality, I make use of the key to retieve a row in O(1) time. For example, if I need to joing table1.sqlID = table2.ID for table1 and table2, I find the table1.sqlID, and use the sqlID as a key to look up in table2's dictionary of table rows. This avoids the need of a nested for loop to check every single row finding a matching row. 
TODO:
